import { ICartSliceInitialState, initialState } from '../cartSlice';
import { createAsyncThunk, SerializedError } from '@reduxjs/toolkit';
import { CartService } from 'services/cart/cart.service';
import { UserOrderModel, CartRequestModel } from 'common/models/common';
import { CartActionTypes } from './cartActionTypes';

export const addCartItem = (state: ICartSliceInitialState, payload: CartRequestModel) => {
...
export const decreaseCartItem = (state: ICartSliceInitialState, payload: CartRequestModel) => {
  if (!(payload.shopItem.item._id in state.itemMap)) {
    return state
  }
  state.cart[state.itemMap[payload.shopItem.item._id]].quantity -= 1
...
export const removeCartItem = (state: ICartSliceInitialState, payload: CartRequestModel) => {
...
const filterCartItems = (state: ICartSliceInitialState, payload: CartRequestModel) => {
